		...
		mApplicationContext.sendOrderedBroadcast(new Intent(MainActivity.DATA_REFRESHED_ACTION), null,	new BroadcastReceiver() {
					...
					public void onReceive(Context context, Intent intent) {
						...
						if (getResultCode() != MainActivity.IS_ALIVE) {
							...
							// If so, create a PendingIntent using the restartMainActivityIntent and set its flags to FLAG_UPDATE_CURRENT
							PendingIntent pendingIntent = PendingIntent.getActivity(context, 1, restartMainActivtyIntent, PendingIntent.FLAG_UPDATE_CURRENT);

							// Uses R.layout.custom_notification for the layout of the notification View.
							// The xml file is in res/layout/custom_notification.xml
							...
							// You will have to set several pieces of information. You can use android.R.drawable.stat_sys_warning for the small icon. You should also setAutoCancel(true).

							Notification.Builder notificationBuilder = new Builder(context);
							notificationBuilder.setSmallIcon(android.R.drawable.stat_sys_warning);
							notificationBuilder.setAutoCancel(true);
							if (success)
								notificationBuilder.setContentText(successMsg);
							else
								notificationBuilder.setContentText(failMsg);
							notificationBuilder.setContentTitle("Tweet download");
